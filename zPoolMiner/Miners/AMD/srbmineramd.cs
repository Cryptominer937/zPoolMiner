...
{
    public class SRBMineramd : Miner
    {
        ...
        public SRBMineramd() : base("SRBMineramd")
        {
        ...
        */
        ...
        private string GetStartCommand(string url, string btcAdress, string worker)
        {
            ...
            if (MiningSetup.CurrentAlgorithmType.Equals(AlgorithmType.cryptonight_upx))
            {
                ...
                try
                {
                    var fs = new FileStream("bin_3rdparty\\SRBMiner\\poolsH.txt", FileMode.Create, FileAccess.Write);
                    var w = new StreamWriter(fs);
        ...
        private string GetStartBenchmarkCommand(string url, string btcAdress, string worker)
        {
            ...
            if (MiningSetup.CurrentAlgorithmType.Equals(AlgorithmType.cryptonight_gpu))
            {
                algo = "cryptonightheavy";
                port = "3364";
                variant = " --ccryptonighttype heavy";
                return $" {variant} --cgpuid {GetDevicesCommandString().TrimStart()} {extras} --apienable --apiport {ApiPort} --cpool loki.miner.rocks:5555 --cwallet L95cF8XmPzzhBA1tkiL1NMijNNbj58vs1iJExK84oi2LKc6RQm2q1Z4PmDxYB7sicHVXY1J5YV9yg6vkMxKpuCK1L1SwoDi --cpassword w={ConfigManager.GeneralConfig.WorkerName.Trim()}";
            }

            return $" {variant} --cgpuid {GetDevicesCommandString().TrimStart()} {extras} --apienable --apiport {ApiPort} --cpool xmr-eu.dwarfpool.com:8005 --cwallet 42fV4v2EC4EALhKWKNCEJsErcdJygynt7RJvFZk8HSeYA9srXdJt58D9fQSwZLqGHbijCSMqSP4mU7inEEWNyer6F7PiqeX.{worker} --cpassword x";
        }
