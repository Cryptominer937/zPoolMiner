...
namespace zPoolMiner.Miners
{
    internal class Mkxminer : Miner
    {
        ...
        private bool BenchmarkException => MiningSetup.MinerPath == MinerPaths.Data.mkxminer;

        protected override int GetMaxCooldownTimeInMilliseconds()
        {
            if (MiningSetup.MinerPath == MinerPaths.Data.mkxminer)
            {
        ...
        protected override string BenchmarkCreateCommandLine(Algorithm algorithm, int time)
        {
            ...
            var CommandLine = " --url " + url + " --user " + Globals.DemoUser + " -p Benchmark" + " -I 23" +
                                  ExtraLaunchParametersParser.ParseForMiningSetup(
                                                                MiningSetup,
                                                                DeviceType.AMD) +
                                  " --device ";
        ...
        protected override bool BenchmarkParseLine(string outdata)
        {
            ...
            if (BenchmarkException)
            {
                if (outdata.Contains("> "))
                {
                    ...
                    var hashspeed = outdata.Substring(st + 6, end - st - 6);
                    /*
