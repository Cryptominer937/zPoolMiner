...
namespace zPoolMiner
{
    ...
    public class NiceHashData
    {
        ...
        private Dictionary<AlgorithmType, zPoolMinerSMA> currentSMA;

        public NiceHashData()
        {
            recentPaying = new Dictionary<AlgorithmType, List<double>>();
            var sma = new Dictionary<AlgorithmType, zPoolMinerSMA>();
            foreach (AlgorithmType algo in Enum.GetValues(typeof(AlgorithmType)))
            {
                if (algo >= 0)
                {
                    sma[algo] = new zPoolMinerSMA
                    {
                        port = (int)algo + 3333,
        ...
        public NiceHashData(ZPoolAlgo[] data)
        {
            ...
            {
                sma[(AlgorithmType)algo.NiceHashAlgoId()] = new zPoolMinerSMA
                {
                    port = algo.Port,
        ...
        public Dictionary<AlgorithmType, zPoolMinerSMA> NormalizedSMA()
        {
